...
class Osciloscopio():
    ...
    def set_scale(self, volts):
        self.instru.write('CURS:HBA:POSITION{}'.format(volts)) # set la escala vertical de tensión

    def get_scale(self):
        return self.instru.query('CURS:HBA:UNI?')

    def __del__(self):
        self.instru.write("UNLOC")
        self.instru.close()
...
print(instr)
osci = Osciloscopio(instr) #meter en el argumento el número de serie

print(osci.identidad())
osci.set_timebase(0.0001) #setear escala de tiempo
#osci.set_scale(2.0) # amplitud máxima en volts
